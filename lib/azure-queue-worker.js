...
function AzureQueueWorker() {
    ...
    self.onCheck = function(cb) { callbackMap.onCheck = cb; };
    ...
    self.run = function() {
        ...
        var config = {
            ...
            qStorageEndpointSuffix: undefined,
            qPolling: undefined,
            qLogger: undefined
        };
        ...
        return q.when(callbackMap.onInitialize(config)).then(function(updatedConfig) {
            ...
            queueListener.onMessage(function(message) {
                ...
                // ensure we have at least empty metainformation
                message.messageMetaInformation = message.messageMetaInformation || {};

                // retrieve a message, first state change needs to be triggered
                return q.when(callbackMap.onState(self.states.received, self.states.none, null, message)).then(function() {

                    // now we transfer the state into running mode
                    return q.when(callbackMap.onState(self.states.running, self.states.received, null, message)).then(function() {
                        ...
                        return tryCatchProcessMessage(message).then(function(postOperation) {
                            ...
                            if (postOperation && postOperation.operation === 'delay') {
                                ...
                                return q.when(callbackMap.onState(self.states.delayed, self.states.running, postOperation.operationInformation, message));
                            } else {
                                // transfer the message into finished mode
                                return q.when(callbackMap.onState(self.states.finished, self.states.running, null, message));
                            }

                        }).catch(function(error) {

                            // transfer the message into failed more
                            return q.when(callbackMap.onState(self.states.failed, self.states.running, error, message));
            ...
            // register the poll handler
            queueListener.onPoll(function(pollCounter) {
                if (callbackMap.onCheck) {
                    return callbackMap.onCheck(pollCounter);
                } else {
                    return q.resolve();
                }
            });
            ...
            // start the listening
            queueListener.listen(updatedConfig.qName, updatedConfig.qStorageAccount, updatedConfig.qStorageSecret, updatedConfig.qPolling, updatedConfig.qLogger, updatedConfig.qStorageEndpointSuffix);
