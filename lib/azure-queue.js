...
function AzureQueue(queueName, storageKey, storageSecret, storageEndpointSuffix) {
    var self = this;
    ...
    function sendMessageInternal(messageType, messageData, messageMetaInformation, nameofQueue, queueService, options) {
        ...
        // Stringify
        var jsonString = JSON.stringify({messageType: messageType, messageData: messageData, messageMetaInformation: messageMetaInformation ? messageMetaInformation : {}});

        // encode to base64
        var base64String = (new Buffer(jsonString)).toString('base64');

        // send to queue
        queueService.createMessage(nameofQueue, base64String, options, function(error, result, response){
            if(error !== null){
                defer.reject(error);
            } else {
                defer.resolve();
    ...
    self.sendMessageWithMetaData = function(messageType, messageData, messageMetaInformation, options) {
        ...
        var queueService = new azure.QueueService(storageKey, storageSecret, undefined, undefined, storageEndpointSuffix).withFilter(retryOperations);
